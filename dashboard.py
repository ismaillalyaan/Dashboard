...
def update_dashboard(country):
    ...
    line_fig = px.line(trend, x='Date', y=['Confirmed', 'Deaths', 'Recovered'], title='Trends Over Time')
    if country == 'World':
        top_countries = data.groupby('Country/Region')[['Confirmed']].sum().nlargest(5, 'Confirmed').reset_index()
        bar_fig = px.bar(top_countries, x='Country/Region', y='Confirmed', title='Top 5 Countries')
    else:
        bar_fig = px.bar(filtered, x='Date', y='Confirmed', title=f'{country} Daily Cases')
    map_fig = px.scatter_geo(filtered, lat='Lat', lon='Long', size='Confirmed', title='Map of Cases')
    return stats, line_fig, bar_fig, map_fig
